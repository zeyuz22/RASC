...
class BalancedBatchSampler(BatchSampler):
    def __init__(self, dataset, n_classes, n_samples):
        ...
        self.used_label_indices_count = {label: 0 for label in
                                         self.labels_set}  # acts as pointer for tracking used indices
        self.count = 0
        self.n_classes = n_classes
        self.n_samples = n_samples  # samples per class
        self.len_dataset = len(self.labels_list)
        self.batch_size = self.n_samples * self.n_classes
        self.all_classes = [i for i in set(self.labels_list)]
    ...
    def __iter__(self):
        ...
        while (self.count + self.batch_size < self.len_dataset) or self.count == 0:
            np.random.shuffle(self.all_classes)
...
def get_sampler(config_data):
    sampler_src_train = BalancedBatchSampler(config_data['dataset'], config_data['num_categories'],
                                             config_data['n_samples'])
